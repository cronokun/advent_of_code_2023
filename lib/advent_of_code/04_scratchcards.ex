defmodule AdventOfCode.Scratchcards do
  ...
  @doc "Returns sum of wining points of all cards"
  def answer(input) do
    input
    |> parse_lines()
    |> get_points()
  ...
  defp parse_lines(input) do
    ...
    |> String.split("\n", trim: true)
    |> Enum.map(fn line ->
      [card, wining, numbers] = String.split(line, [": ", " | "])
      {parse_card_number(card), parse_numbers(wining), parse_numbers(numbers)}
    end)
  end
  ...
  defp get_points(lines),
    do: Enum.map(lines, fn {_card, win, nums} -> calc_points({win, nums}, 0) end)

  defp calc_points({_wining, []}, total), do: total

  defp calc_points({wining, [n | nums]}, total) do
    new_total =
      if Enum.member?(wining, n), do: increment_total(total), else: total

    calc_points({wining, nums}, new_total)
  end

  defp increment_total(0), do: 1
  defp increment_total(n), do: n * 2

  defp get_total(points), do: Enum.sum(points)

  @doc "How many total scratchcards you win"
  ...
  defp process_cards(cards) do
    for {cnum, win, nums} <- cards do
      a = MapSet.new(win)
      b = MapSet.new(nums)
      wins_count = MapSet.intersection(a, b) |> MapSet.size()

      wins =
        case wins_count do
          0 -> []
          n -> Range.new(cnum + 1, cnum + n) |> Enum.to_list()
        end
  ...
  defp build_map([{n, wins} | rest], acc) do
    known_sum = Enum.reduce(wins, 1, fn prev_n, sum -> sum + Map.get(acc, prev_n, 0) end)
    acc = Map.update(acc, n, known_sum, fn s -> s + known_sum end)
    build_map(rest, acc)
  end

  defp calculate_sum(map), do: map |> Map.values() |> Enum.sum()
end
