defmodule AdventOfCode.Scratchcards do
  ...
  defp parse_lines(input) do
    ...
    |> String.split("\n", trim: true)
    |> Enum.map(fn line ->
      [_card, wining, numbers] = String.split(line, [": ", " | "])
      {parse_numbers(wining), parse_numbers(numbers)}
    end)
  end
  ...
  defp get_points(lines), do: Enum.map(lines, &calc_points/1)

  defp calc_points(line, total \\ 0)
