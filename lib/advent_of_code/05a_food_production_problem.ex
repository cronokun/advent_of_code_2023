defmodule AdventOfCode.FoodProductionProblem do
  @moduledoc """
  ...
  only turned off the water a few days... weeks... oh no." His face sinks into
  ...
  with each kind of soil, what type of water to use with each kind of fertilizer,
  ...
  seed number 98 corresponds to soil number 50 and that seed number 99 corresponds
  to soil number 51.
  ...
  **What is the lowest location number that corresponds to any of the initial seed numbers?**
  ...
  @doc "Lowest location number that corresponds to any of the initial seed numbers"
  def answer(input) do
    {seeds, maps} = parse_input(input)

    process_seeds(seeds, maps) |> Enum.min()
  end
  ...
  defp parse_seeds(<<"seeds: ", seeds::binary>>), do: string_to_numbers(seeds)

  defp parse_map(map) when is_binary(map) do
    [_name | ranges] = String.split(map, "\n", trim: true)

    Enum.map(ranges, fn str ->
  ...
  defp process_seeds(seeds, maps), do: Enum.map(seeds, &get_location_number(&1, maps))
  defp get_location_number(n, maps), do: Enum.reduce(maps, n, &process_map/2)

  defp process_map(map, n) do
    Enum.reduce_while(map, n, fn {source, dest, range}, num ->
      if num >= source and num <= source + range - 1 do
        {:halt, num - source + dest}
      else
        {:cont, num}
      end
    end)
