defmodule AdventOfCode.LensLibrary do
  ...
  defp do_hash(str, hash \\ 0, total \\ 0)
  defp do_hash("", hash, total), do: total + hash
  defp do_hash("," <> rest, hash, total), do: do_hash(rest, 0, total + hash)
  defp do_hash("\n" <> rest, hash, total), do: do_hash(rest, hash, total)

  defp do_hash(<<char::utf8, rest::binary>>, hash, total),
    do: do_hash(rest, rem((char + hash) * @multiplier, @modulo), total)
end
