defmodule AdventOfCode.LensLibrary do
  ...
  The newly-focused parabolic reflector dish is sending all of the collected light to a point
  ...
  must be the room that collects all of the light from outside. As you admire the large assortment
  of lenses available to further focus the light, the reindeer brings you a book titled
  ...
  0 to 255. To run the HASH algorithm on a string, start with a current value of 0. Then, for
  ...
  You convince the reindeer to bring you the page; the page confirms that your HASH algorithm
  is working.

  The book goes on to describe a series of 256 boxes numbered 0 through 255. The boxes are
  arranged in a line starting from the point where light enters the facility. The boxes have
  holes that allow light to pass from one box to the next all the way down the line.
  ...
  focus light passing through the box. The side of each box has a panel that opens to allow you
  to insert or remove lenses as necessary.

  Along the wall running parallel to the boxes is a large library containing lenses organized
  by focal length ranging from 1 through 9. The reindeer also brings you a small handheld
  label printer.
  ...
  To confirm that all of the lenses are installed correctly, add up the focusing power of all of
  ...
  **What is the focusing power of the resulting lens configuration?**
  ...
  @doc "Sum of all initialization sequence hashes"
  def answer(input) do
    input
    |> String.trim()
    |> String.split(",")
    |> Enum.reduce(0, fn instruction, sum -> sum + do_hash(instruction) end)
  end

  @doc "Sum of all lenses focusing power"
  ...
  defp arrange_lenses(instructions) do
    Enum.reduce(instructions, %{}, fn str, acc ->
      case parse_instruction(str) do
        {:put, block, label, focal} -> put_lens(acc, block, label, focal)
        {:remove, block, label} -> remove_lens(acc, block, label)
      end
    end)
  end

  defp put_lens(acc, block, label, focal) do
    Map.update(acc, block, [{label, focal}], fn list ->
      if Keyword.has_key?(list, label) do
        Keyword.replace(list, label, focal)
      else
        Keyword.put(list, label, focal)
      end
    end)
  end

  defp remove_lens(acc, block, label) do
    Map.update(acc, block, [], fn list -> Keyword.delete(list, label) end)
  end

  defp calculate_focusing_power(boxes) do
    Enum.reduce(boxes, 0, fn {i, box}, total ->
      total + (i + 1) * box_focusing_power(box)
    end)
  end

  defp box_focusing_power(list) do
    list
    |> Keyword.values()
    |> Enum.reverse()
    |> Enum.with_index(1)
    |> Enum.reduce(0, fn {focal, i}, sum -> sum + i * focal end)
  end

  defp do_hash(str, hash \\ 0)
  defp do_hash("", hash), do: hash
  defp do_hash(<<c::utf8, rest::binary>>, hash), do: do_hash(rest, rem((c + hash) * 17, 256))

  # --- Parser ----

  defp parse_input(input), do: input |> String.trim() |> String.split(",")

  @regex ~r/([a-z]+)([-=])(\w*)/

  defp parse_instruction(instruction) do
    case Regex.run(@regex, instruction, capture: :all_but_first) do
      [str, "=", number] -> {:put, do_hash(str), String.to_atom(str), String.to_integer(number)}
      [str, "-", ""] -> {:remove, do_hash(str), String.to_atom(str)}
    end
  end
end
