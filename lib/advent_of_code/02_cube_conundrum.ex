defmodule AdventOfCode.CubeConundrum do
  ...
  **What is the sum of the power of these sets**?
  ...
  @doc "Sum of the IDs of valid games"
  def answer(input) do
    input
    |> String.split("\n", trim: true)
  ...
  def second_answer(input) do
    ...
    |> String.split("\n", trim: true)
    |> Enum.map(&parse_line/1)
    |> Enum.map(fn {_id, sets} -> get_minimal_rgb_set(sets) end)
    |> Enum.map(&calculate_set_power/1)
    |> Enum.sum()
  end

  defp get_minimal_rgb_set(sets, acc \\ {0, 0, 0})
  defp get_minimal_rgb_set([], acc), do: acc

  defp get_minimal_rgb_set([{r, g, b} | rest], {minr, ming, minb}) do
    acc = {max(r, minr), max(g, ming), max(b, minb)}
    get_minimal_rgb_set(rest, acc)
  end

  defp calculate_set_power(set), do: Tuple.product(set)
end
