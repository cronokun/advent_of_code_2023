defmodule AdventOfCode.ParabolicReflectorDish do
  ...
  defp tilt_north(grid) do
    grid |> Enum.map(&do_tilt/1) |> flip() |> Enum.reverse()
  end

  defp do_tilt(list, tmp \\ [], acc \\ [])
  defp do_tilt([], tmp, acc), do: acc ++ tmp
  defp do_tilt(["O" | rest], tmp, acc), do: do_tilt(rest, tmp, acc ++ ["O"])
  defp do_tilt(["#" | rest], tmp, acc), do: do_tilt(rest, [], acc ++ tmp ++ ["#"])
  defp do_tilt(["." | rest], tmp, acc), do: do_tilt(rest, tmp ++ ["."], acc)

  defp calculate_load(grid) do
    Enum.reduce(grid, {0, 1}, fn line, {total, ln} ->
      load = Enum.count(line, &(&1 == "O"))
      {total + ln * load, ln + 1}
    end)
    |> elem(0)
  end
