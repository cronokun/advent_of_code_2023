defmodule AdventOfCode.SandSlabs do
  ...
  def answer(input) do
    ...
    length(grid) - MapSet.size(props)
  end

  defp apply_gravity(bricks, grid \\ [], acc \\ MapSet.new())
  defp apply_gravity([], grid, acc), do: {grid, acc}

  defp apply_gravity([brick | rest], grid, acc) do
    {new_brick, props} = put_down_brick(brick, grid)
    new_grid = [new_brick | grid]
    new_acc = maybe_update_props(acc, props)
    apply_gravity(rest, new_grid, new_acc)
  end
  ...
  defp put_down_brick(brick, grid) do
    ...
    |> case do
      {0, []} -> {update_brick(brick, 0), nil}
      {z, [prop]} -> {update_brick(brick, z + 1), prop}
      {z, _props} -> {update_brick(brick, z + 1), nil}
    end
  end
  ...
  defp maybe_update_props(acc, nil), do: acc
  defp maybe_update_props(acc, {_, _, label}), do: MapSet.put(acc, label)
  ...
  defp parse_lines(lines, acc \\ [], n \\ 1)
  defp parse_lines([], acc, _n), do: Enum.reverse(acc)

  defp parse_lines([line | rest], acc, n),
    do: parse_lines(rest, [parse_line(line, n) | acc], n + 1)
